...
export function shouldSkipTransform(mapper: Mapper | undefined, to: unknown, from: unknown): boolean {
  return !mapper || !to || !from;
}

export function transformArray(value: unknown, mapper: Mapper | undefined, to: any, from: any, options?: MapOptions) {
  if (!Array.isArray(value)) return value;
  return mapper?.mapArray(value, to, from, options);
}

export function getTransformOptions(options?: { isArray?: boolean; mapperName?: string } & MapOptions): {
  mapperName?: string;
  isArray: boolean;
