...
export const isArrayNotEmpty = (val: any): boolean => Array.isArray(val) && hasLength(val);

export const isArrayWithLength = (val: any, length: number) => isArrayNotEmpty(val) && val.length === length;

export const isArrayStrings = (val: any): boolean =>
  isArrayNotEmpty(val) && (val as string[]).every((v) => isStringFull(v));
...
export const hasValue = (val: any): boolean =>
  isArrayNotEmpty(val) ? (val as any[]).every((o) => isValue(o)) : isValue(val);
